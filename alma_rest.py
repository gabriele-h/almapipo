...
def restore_records_from_db_via_api_for_csv_list(csv_path: str, api: str, record_type: str):
    ...
    * Set status of API call in job_status_per_id
    ...
    for alma_id, in list_of_ids:
        record_data = xml_extract.extract_xml_from_fetched_records(alma_id)
    ...
    * Save the response from the API in table fetched_records
    ...
    for alma_id, in list_of_ids:
        ...
        if record_data is None:
            ...
            alma_record_json = loads(str(record_data, 'utf-8'))
            db_read_write.add_fetched_record_to_session(alma_id, alma_record_json, job_timestamp, db_session)
    db_session.commit()
