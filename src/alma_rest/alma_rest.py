...
def update_records_from_db_via_api_for_csv_list(
        ...
        api: str,
        record_type: str,
        manipulation: Callable[[str, str], str]) -> None:
    """
    ...
    for alma_id, in list_of_ids:
        ...
        if not record_data:
            ...
            if not new_record_data:
                ...
                db_read_write.update_job_status_for_alma_id('error', alma_id, job_timestamp, db_session, 'PUT')
                logger.error(f"{manipulation} did not carry out a change of {alma_id}.")
            else:
                db_read_write.add_put_post_response_to_session(alma_id, record_data, job_timestamp, db_session)
